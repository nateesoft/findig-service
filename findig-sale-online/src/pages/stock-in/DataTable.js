...
const DataTable = ({
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
      <head>
        <title>รายการข้อมูลการโอนสินค้าเข้า</title>
        <meta charset="utf-8">
        <style>
      ...
      <body>
        ...
        <h1>รายการข้อมูลการโอนสินค้าเข้า</h1>
        <table>
          <thead>
            <tr>
              <th>เลขที่เอกสาร</th>
              <th class="text-center">วันที่โอนสินค้าเข้า</th>
              <th class="text-right">จำนวนสินค้า</th>
              <th class="text-center">พนักงานทำรายการ</th>
              <th class="text-center">สาขาที่โอนเข้า</th>
          ...
          <tbody>
            ${sortedSales.map(item => `
              <tr>
                ...
                <td class="text-center">${item.branch_stock_in || ''}</td>
  ...
  const handleExportExcel = () => {
    ...
    // เพิ่ม headers
    worksheetData.push([
      'เลขที่เอกสาร', 'วันที่โอนสินค้าเข้า', 'จำนวนสินค้า', 'พนักงานทำรายการ', 'สาขาที่โอนเข้า', 'สถานะ POST'
    ]);
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'รายการข้อมูลการโอนสินค้าเข้า');
    
    // ดาวน์โหลดไฟล์
    XLSX.writeFile(wb, `รายการข้อมูลการโอนสินค้าเข้า_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
          ...
          <thead className={getThemeClasses("tableHeader", currentTheme)}>
            <tr>
              ...
              <th
                ...
                onClick={() => handleSort('branch_stock_in')}
              >
                <div className="flex items-center">
                  สาขาที่โอนเข้า
                  {getSortIcon('branch_stock_in')}
                </div>
              </th>
              <th
                className={`px-6 py-3 text-left text-xs font-medium ${getThemeClasses(
            ...
            {currentItems.length > 0 ? (
              currentItems.map((stock_in) => (
                ...
                >
                  ...
                  >
                    {moment(stock_in.document_date).format(
                      "DD/MM/YYYY HH:mm:ss"
                    )}
                  </td>
                  <td
                  ...
                  >
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${
                        stock_in.post_status === "Y"
                          ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200"
                          : stock_in.post_status === "N"
                          ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200"
                          : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                      }`}
                    >
                      {stock_in.post_status}
                    </span>
                  </td>
                  <td
                  ...
                  >
                    <div className="flex justify-center space-x-2">
                      <button
                        onClick={() => handleReviewSale(stock_in.id)}
                      ...
                      {stock_in.post_status !== "Y" && 
                        <button
                          onClick={() => handleEditSale(stock_in.id)}
                          className={`p-2 rounded-lg ${getThemeClasses(
                            "textMuted",
                            currentTheme
                          )} hover:${getThemeClasses(
              ...
              <tr>
                ...
                >
                  <div className="flex flex-col items-center justify-center space-y-2">
                    ...
                    <p>ไม่พบข้อมูลการโอนสินค้าเข้าที่ตรงกับเงื่อนไขการค้นหา</p>
