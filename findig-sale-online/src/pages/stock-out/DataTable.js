...
const DataTable = ({
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
      <head>
        <title>รายการข้อมูลการโอนสินค้าออก</title>
        <meta charset="utf-8">
        <style>
      ...
      <body>
        ...
        <h1>รายการข้อมูลการโอนสินค้าออก</h1>
        <table>
          <thead>
            <tr>
              <th>เลขที่ใบเสร็จ</th>
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'รายการข้อมูลการโอนสินค้าออก');
    
    // ดาวน์โหลดไฟล์
    XLSX.writeFile(wb, `รายการข้อมูลการโอนสินค้าออก_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <h1 className={`text-2xl font-bold ${getThemeClasses('textPrimary', currentTheme)}`}>เมนูโอนสินค้าออก</h1>
        </div>
        ...
        <table className="w-full">
          ...
          >
            {currentItems.length > 0 ? (
              currentItems.map((stock_out) => (
                <tr
                  key={stock_out.billno}
                ...
                >
                  ...
                  >
                    {moment(stock_out.document_date).format(
                      "DD/MM/YYYY HH:mm:ss"
                    )}
                  </td>
                  <td
                  ...
                  >
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${
                        stock_out.post_status === "Y"
                          ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200"
                          : stock_out.post_status === "N"
                          ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200"
                          : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                      }`}
                    >
                      {stock_out.post_status}
                    </span>
                  </td>
                  <td
                  ...
                  >
                    <div className="flex justify-center space-x-2">
                      <button
                        onClick={() => handleReviewSale(stock_out.id)}
                      ...
                      {stock_out.post_status !== "Y" && 
                        <button
                          onClick={() => handleEditSale(stock_out.id)}
                          className={`p-2 rounded-lg ${getThemeClasses(
                            "textMuted",
                            currentTheme
                          )} hover:${getThemeClasses(
              ...
              <tr>
                ...
                >
                  <div className="flex flex-col items-center justify-center space-y-2">
                    ...
                    <p>ไม่พบข้อมูลการโอนสินค้าออกที่ตรงกับเงื่อนไขการค้นหา</p>
