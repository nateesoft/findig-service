const getData = async ({ payload, db }) => {
  try {
    ...
    const sql = `select * from stock_out_details`
...
const getDataById = async ({ payload, db }) => {
  try {
    ...
    const { id } = payload
    const sql = `select * from stock_out_details where id=?`
...
const getDataByBillNo = async ({ payload, db }) => {
  try {
    ...
    const sql = `select * from stock_out_details where billno=?`
...
const saveData = async ({ payload, db }) => {
  try {
    ...
    const sql = `INSERT INTO stock_out_details
...
const updateData = async ({ payload, db }) => {
  try {
    ...
    const sql = `UPDATE stock_out_details
...
const deleteData = async ({ payload, db }) => {
  try {
    ...
    const { id } = payload
    const sql = `DELETE FROM stock_out_details WHERE id=?`
    const results = await db.pos.query(sql, [id])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
...
const deleteDataByBillNo = async ({ payload, db }) => {
  try {
    ...
    const { billno } = payload
    const sql = `DELETE FROM stock_out_details WHERE billno=?`
    const results = await db.pos.query(sql, [billno])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
