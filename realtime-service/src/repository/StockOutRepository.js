...
const getData = async ({ payload, db }) => {
  try {
    ...
    const sql = `select * from stock_out where branch_code=?`
...
const getDataForDashboard = async ({ payload, db }) => {
  try {
    ...
    const { branch_code } = payload
    const sql = `select * from stock_out where branch_code=? limit 0, 10`
    const results = await db.pos.query(sql, [branch_code])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
...
const getDataById = async ({ payload, db }) => {
  try {
    ...
    const { id } = payload
    const sql = `select * from stock_out where id=?`
...
const getAllBillNotProcess = async ({ payload, db }) => {
  try {
    ...
    const { branch_code } = payload
    const sql = `select * from stock_out where branch_code=? and post_status=?`
...
const saveData = async ({ payload, db }) => {
  try {
    ...
    const { id, billno, document_date, branch_code, post_status, 
      emp_code, update_date, total_item, emp_code_update, branch_stock_out_code } = payload
    const sql = `INSERT INTO stock_out
                (id, billno, document_date, branch_code, post_status, 
                emp_code, update_date, total_item, emp_code_update, branch_stock_out_code)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    const results = await db.pos.query(sql, 
      [id, billno, document_date, branch_code, post_status, emp_code, update_date, total_item, emp_code_update, branch_stock_out_code])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
  }
...
const updateData = async ({ payload, db }) => {
  try {
    ...
    const { id, branch_code, post_status, emp_code_update, update_date, total_item, branch_stock_out_code } = payload
    const sql = `UPDATE stock_out
                SET branch_code=?, post_status=?, emp_code_update=?, update_date=?, total_item=?, branch_stock_out_code=?
                WHERE id=?`
    const results = await db.pos.query(sql, 
      [branch_code, post_status, emp_code_update, update_date, total_item, branch_stock_out_code, id])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
  }
...
const processStockFromSale = async ({ payload, db }) => {
  try {
    ...
    const { id } = payload
    const sql = `UPDATE stock_out SET post_status='Y' WHERE id=?`
    const results = await db.pos.query(sql, [id])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
...
const deleteData = async ({ payload, db }) => {
  try {
    ...
    const { id } = payload
    const sql = `DELETE FROM stock_out WHERE id=?`
    const results = await db.pos.query(sql, [id])
    return results
  } catch (error) {
    throw new Error(`Database query failed: ${error.message}`)
